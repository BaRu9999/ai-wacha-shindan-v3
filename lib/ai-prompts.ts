 ...
 *  - 文章はさらに短く（仕様14）。AI が主役にならないよう、項目ごとに文数の上限を決めている。
 *  - 指定 JSON 以外は出力させない。
 */
...
export const SYSTEM_PROMPT = [
  ...
  "文章は短いほどよい。各項目は指定された文数を超えないこと。全角30〜70字程度を目安にする。",
  "回答内容の具体語を1つ以上さりげなく織り込む。長く書いて「AIが頑張った感」を出す必要はない。",
...
export function buildUserPrompt(params: UserPromptParams): string {
  ...
  return [
    ...
    "",
    "作成する項目（文数は厳守。超えないこと）:",
    "- summary: 最大2文。今日のこのタイプらしさを、回答の具体語を交えて短く。",
    "- hiddenInsight: 1〜2文。隠れタイプがどんな時に表れるか。",
    "- today: 必ず1文だけ。回答をふまえた、そっと寄り添う一言。指示や助言の口調にしない。",
    "- recommendationReason: 1〜2文。おすすめの組み合わせが今日の気分に合う理由。誇張しない。",
    "- word: 和ことば（四字熟語など、既存の言葉から1つ）。単語のみ。",
    "- wordMeaning: 1文。その和ことばを、今日のこの人に向けてやさしく言い換える。",
  ].join("\n");
}
