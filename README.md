...
- 「今日のおすすめ」は `productSets`（同ファイル内）で商品を2点程度に組み合わせたもの。
  新しい組み合わせを増やしたら、`lib/recommendation.ts` の `setPlanByType` に
  どの「傾向（甘め・軽め・ドリンク中心）」で使うかを追記してください。
- 写真は `public/menu/` に追加し、`data/products.ts` の `image` を差し替えます（`next/image` が自動最適化）。

> 現行版はセット単位の価格しか持っていなかったため、単品価格は暫定値です
> （コード内に `// 暫定` と明記）。実際の税込価格に置き換えてから公開してください。
...
- 表示中の単品価格・写真は初期値です。実際の商品名・価格・写真に差し替えてから公開してください。
